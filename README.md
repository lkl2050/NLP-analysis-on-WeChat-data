...
The above analysis with LSTM used only tokenized words (i.e., count encoding) as the input, they did no capture the semantic information among the words. This analysis adopted the pretrained BERT model with some fine tunining at the output layer, trying to achieve a better prediction result.
